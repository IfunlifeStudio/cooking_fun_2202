...
public class GreenTeaMachine : Machine
{
    ...
    [SerializeField] private GameObject instantCookEffect, autoServeEffect;
    ...
    public override bool FillIngredent()
    {
        if (state == MachineStates.Standby)
        {
            ...
            if (LevelDataController.Instance.CheckItemActive((int)ItemType.Instant_Cook))
            {
                ...
                GameObject instantEffect = Instantiate(instantCookEffect, transform.position + new Vector3(0, 2.8f, 0), instantCookEffect.transform.rotation);
