...
public class EventLosePanel : LosePanelController
{
    ...
    public new void Init(int objectiveType, string objectiveProgress, bool isConditionViolate, int conditionType)
    {
    ...
    public new void OnClickReplay()
    {
        if (DataController.Instance.IsUnlimitedEnergy())
        {
            string currentScene = EventDataController.Instance.GetGamePlayScene(EventDataController.Instance.currentChapter);
