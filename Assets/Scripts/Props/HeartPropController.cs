...
public class HeartPropController : MonoBehaviour
{
    private int value = 0;
    public void Spawn(int value, Vector3 position, Transform target, UnityAction action = null)
    {
        GameObject go = Instantiate(gameObject);
        go.GetComponent<HeartPropController>().Init(go,value, position, target,action);
    }
    ...
    public void Dispose()
    {
        PropFactory.Instance.DisposeHeart(this);
